...
- [Firecrawl](https://www.firecrawl.dev): works without a key on the free keyless tier; an API key unlocks your plan's credits and full toolset (about 2 credits per lead)
...
2. Enter your Instantly API key when prompted during install. Create it under Settings > Integrations > API with these scopes:
   - `campaigns:create`
   - `campaigns:read`
   - `leads:create`
   - `leads:read`
   ...
   Copy the key right away; Instantly only shows it once. The Firecrawl API key prompt is optional. Keys are stored in your system's secure credential store. To change them later, run `/plugin configure cold-email@fisher-research`.
